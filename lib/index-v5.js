...
function render$1(config) {
	const badgeNode = node('image', {
		...
		url: `http://img.shields.io/badge/status-${ config.text }-${ config.color }.svg?style=${ config.style }`
	});
...
function renderIconSVG(id) {
	const iconSource = fs.readFileSync(path.resolve(__dirname, `../icons/${ id }.svg`));
...
function render(context) {
	let asAST = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
